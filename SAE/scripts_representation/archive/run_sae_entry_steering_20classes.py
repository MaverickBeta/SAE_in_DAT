...
Usage:
    ...
    python run_sae_entry_steering_20classes.py
"""
...
GLOBAL_JSON = Path("/Data_share/hongyi/DAT/SAE/results_representation/results_20classes/global_candidates.json")
OUT_DIR = Path("/Data_share/hongyi/DAT/SAE/results_representation/steering_20classes")
...
def main():
    print("\n" + "=" * 70)
    print("CROSS-CLASS STEERING VALIDATION (4 conditions)")
    print("=" * 70)
    ...
    for wnid, cls_idx, name in CLASSES:
        ...
        # 2. Global
        print("  [2/4] Global 26 entries...")
        global_hook = make_steering_hook(global_targets)
        global_acc = evaluate(images, global_hook)

        # 3. Random A
        clean_npy = FEAT_DIR / f"clean_{wnid}_cls{cls_idx}_stage3_k256_features.npy"
        random_a_targets = build_random_a_targets(clean_npy, n=26)
        ...
        # 4. Random B
        print("  [4/4] Random B (shuffled targets)...")
        ...
        print(f"  Control: {control_acc:.4f}  Global: {global_acc:.4f}  "
              f"RandomA: {ra_str}  "
    ...
    print("=" * 70)
    print(f"{'Class':>15} {'N':>3} {'Ctrl':>6} {'Global':>7} {'RandA':>7} {'RandB':>7} {'G-C':>6} {'G-RA':>6} {'G-RB':>6}")
    print("-" * 70)
    ...
    for r in results:
        ...
        print(f"{r['name']:>15} {r['n_adv_samples']:>3} "
              f"{r['control_acc']*100:>5.1f}% {r['global_acc']*100:>6.1f}% "
              f"{ra_str} "
