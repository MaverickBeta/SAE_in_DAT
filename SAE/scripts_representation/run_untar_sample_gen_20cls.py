#!/usr/bin/env python3
"""
Batch run untar_sample_gen_train.py for 20 representative ImageNet classes.

Usage:
    cd /Data_share/hongyi/DAT/SAE/scripts_representation
    python run_untar_sample_gen_20cls.py

Data source:
    /Data_share/hongyi/imagenet/train/{wnid}/  (100 JPEGs per class)

Output:
    /Data_share/hongyi/DAT/SAE/results_representation/untar_samples_20cls_train/
...
#   CUDA_VISIBLE_DEVICES=7 python run_untar_sample_gen_20cls.py
os.environ.setdefault("CUDA_VISIBLE_DEVICES", "7")

# ── Config ──────────────────────────────────────────────────────────
IMAGENET_TRAIN = Path("/Data_share/hongyi/imagenet/train")
OUTPUT_ROOT = Path("/Data_share/hongyi/DAT/SAE/results_representation/untar_samples_20cls_train")
UNTAR_SCRIPT = Path(__file__).resolve().parent / "untar_sample_gen_train.py"
...
def run_class(wnid: str, cls_idx: int, name: str) -> bool:
    source_dir = IMAGENET_TRAIN / wnid
