...
For each class, extracts SAE latent features for:
  ...
  2. Adv images:     untar_samples_20cls/{run_name}/

Usage:
    cd /Data_share/hongyi/DAT/SAE/scripts_representation
    python run_sae_feature_extract_train.py
"""
...
# ── Paths ───────────────────────────────────────────────────────────
IMAGENET_TRAIN = Path("/Data_share/hongyi/imagenet/train")
ADV_ROOT = Path("/Data_share/hongyi/DAT/SAE/results_representation/untar_samples_20cls")
FEAT_SCRIPT = Path(__file__).resolve().parent / "sae_feature_extract.py"
OUTPUT_DIR = Path("/Data_share/hongyi/DAT/SAE/results_representation/untar_samples_20cls_latent")
...
def extract_features(input_dir: Path, run_name: str, max_images: int = 0) -> bool:
...
def main():
    ...
    for wnid, cls_idx, name in CLASSES:
        ...
        adv_dir = ADV_ROOT / adv_run_name
        
        # 1. Clean features (first 100 images, aligned with adv generation)
        clean_input = IMAGENET_TRAIN / wnid
        clean_run_name = f"clean_{wnid}_cls{cls_idx}_stage3_k256"
        ok = extract_features(clean_input, clean_run_name, max_images=100)
        if ok:
            success += 1
        else:
            failed += 1

        # 2. Adv features (ALL adversarial images: succ + fail)
        adv_run_name_feat = f"adv_{wnid}_cls{cls_idx}_stage3_k256"
        ok = extract_features(adv_dir, adv_run_name_feat, max_images=100)
        if ok:
            success += 1
        else:
            failed += 1
