...
ATTACK_OUTPUT_ROOT = "/Data_share/hongyi/DAT/data/ImageNet/val_attacks_sl2py"
...
SKIP_FAB = os.getenv("SKIP_FAB", "1") == "1"
...
FIXED_ATTACKS = {
    "pgd_untar_sl2py": {
        "builder": lambda m: torchattacks.PGD(m, eps=12 / 255, alpha=2 / 255, steps=40),
        "batch_size": 16,
        "params": {"eps": 12 / 255, "alpha": 2 / 255, "steps": 40},
    },
    "cw_untar_sl2py": {
        "builder": lambda m: torchattacks.CW(m, c=1.0, kappa=0, steps=100),
        "batch_size": 16,
        "params": {"c": 1.0, "kappa": 0, "steps": 100},
    },
    "apgd_ce_untar_sl2py": {
        "builder": lambda m: torchattacks.APGD(m, eps=24 / 255, steps=300, loss="ce"),
        "batch_size": 8,
        "params": {"eps": 24 / 255, "steps": 300, "loss": "ce"},
    },
    "fab_untar_sl2py": {
        "builder": lambda m: torchattacks.FAB(m, eps=16 / 255, steps=FAB_STEPS),
        "batch_size": FAB_BATCH_SIZE,
        "params": {"eps": 16 / 255, "steps": FAB_STEPS},
    },
    "square_untar_sl2py": {
        "builder": lambda m: torchattacks.Square(m, eps=24 / 255, n_queries=SQUARE_N_QUERIES),
        "batch_size": 2,
        "params": {"eps": 24 / 255, "n_queries": SQUARE_N_QUERIES},
    },
}
...
def run_untargeted_attacks():
    ...
    print(f"FAB_STEPS={FAB_STEPS}, FAB_BATCH_SIZE={FAB_BATCH_SIZE}, SQUARE_N_QUERIES={SQUARE_N_QUERIES}")
    ...
    source_dir = os.path.join(IMAGENET_VAL_DIR, SEA_LION_WNID)
