#!/usr/bin/env python3
"""
批量 APGD-CE 攻击 SAE-NCM 模型：多 GPU 并行。
...
def worker(gpu_id: int, task_queue, results_list, config: dict):
    ...
    while True:
        ...
        cmd = [
            sys.executable,
            str(SCRIPT_DIR / "baseline_attack_ncm.py"),
...
def aggregate_results(output_dir: Path):
    """汇总所有 ncm_attack_results_*.json"""
    all_results = []
    for p in sorted(output_dir.glob("ncm_attack_results_*.json")):
...
def main():
    parser = argparse.ArgumentParser(description="Batch APGD-CE on SAE-NCM model")
    ...
    parser.add_argument("--output-dir", type=str,
                        default=str(SCRIPT_DIR / "adv_samples"))
