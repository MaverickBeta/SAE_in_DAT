#!/usr/bin/env python3
"""
批量 L2 PGD-CE 攻击 SAE-NCM 模型：多 GPU 并行。
"""
...
def worker(gpu_id: int, task_queue, results_list, config: dict):
    ...
    while True:
        ...
        cmd = [
            sys.executable,
            str(SCRIPT_DIR / "baseline_attack_ncm_l2.py"),
        ...
        if config.get("class_npz"):
            cmd += ["--class-npz", str(config["class_npz"])]
        cmd += ["--skip-existing"]
...
def aggregate_results(output_dir: Path):
    all_results = []
    for p in sorted(output_dir.glob("ncm_l2_attack_results_*.json")):
...
def main():
    parser = argparse.ArgumentParser(description="Batch L2 PGD-CE on SAE-NCM model")
    ...
    parser.add_argument("--gpus", type=int, nargs="+", default=[3, 4, 5, 7])
    parser.add_argument("--class-list", type=str, default=None)
    parser.add_argument("--output-dir", type=str,
                        default=str(SCRIPT_DIR / "adv_samples_l2"))
    parser.add_argument("--sae-ckpt", type=str, default=None,
                        help="Path to SAE checkpoint (overrides default)")
    parser.add_argument("--class-npz", type=str, default=None,
                        help="Path to class statistics npz (overrides default)")
    ...
    config = {
        ...
        "class_npz": Path(args.class_npz) if args.class_npz else None,
    }
